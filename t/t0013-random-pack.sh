#!/bin/bash
#
# Randomized operation log focused on pack boundary behavior and compatibility checks.
        ...
        case "$op" in
            ...
            4)
                if [ "${#branches[@]}" -gt 1 ]; then
                    ...
                    next_id[$cur]=$((next_id[$cur] + 1))
                    last_file[$cur]="$file"
                    repo_file_count["$cur"]=$((repo_file_count["$cur"] + 1))
                    echo "commit-new $file seed${seed}_${step}" >> "$out"
                fi
                ;;
            5)
...
test_expect_success 'random pack-boundary ops seed=1001' '
    run_case 1001 40 4
'

test_expect_success 'random pack-boundary ops seed=1002' '
    run_case 1002 40 4
'
